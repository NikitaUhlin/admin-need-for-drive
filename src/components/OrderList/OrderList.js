...
const OrderList = () => {
    ...
    const onOrderAccept = (id) => {
        ...
        dispatch(changeOrder(id, { orderStatusId: statuses.find((item) => "Подтвержденные" === item.name).id }))
    }
    const onOrderCancel = (id) => dispatch(changeOrder(id, { orderStatusId: statuses.find((item) => "Отмененые" === item.name).id }))
